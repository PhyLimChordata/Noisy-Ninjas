...
// // server.js
// var serverss = require('http').createServer(app); 
// var io = require('socket.io')(serverss, {
// allowEIO3: true,
//   cors: {
//     origin: "https://noisy-ninjas.nn.r.appspot.com",
//     methods: ["GET", "POST"],
...
io.on("connection", (socket) => {
  ...
  io.sockets.emit("left_queue", JSON.stringify({ninjaQueue: q.ninjas, monsterQueue: q.monsters}));
  })

  socket.on("enter", (_data) => {
    let data = JSON.parse(_data)
    console.log(["draco", "screamer", "tiny"].includes(data.skin));
      ...
      io.sockets.emit("entered_queue", JSON.stringify({ninjaQueue: q.ninjas, monsterQueue: q.monsters, queue: q}));    
  })
  ...
  socket.on("matchFound", (_data) => {
    ...
    io.sockets.emit("foundMatch", JSON.stringify({matchID: data.matchID, ninjaQueue: data.ninjaQueue, monsterQueue: data.monsterQueue}))
  })

  socket.on("create", (_data) => {
    let data = JSON.parse(_data)
  ...
  socket.on("update", (_data) => {
    ...
    currMatch = Matches.find(e=> e.matchId === data.matchId)
        ...
        if(ready){
            console.log(data.matchId);
            io.sockets.emit("ready", JSON.stringify({message: "ready", data: data.matchId}))
  ...
  socket.on("death", (_data) => {
    ...
    currMatch = Matches.find(e => e.matchId === data.matchId)
        ...
        currMatch.user.pop(currPlayer);
        if (["draco", "screamer", "tiny"].includes(data.skin)) {
          io.sockets.emit("ninjas won", JSON.stringify({data: currMatch}));
        }
        else if (currMatch.user.length === 1) {
          io.sockets.emit("monster won", JSON.stringify({data: currMatch}));
        }
  })
})
...
serve.listen(3210, () => {
  console.log("its running on port " + 3210)
})
